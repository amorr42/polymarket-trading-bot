...
class GammaClient(ThreadLocalSessionMixin):
    ...
    def _normalize_params(params: Dict[str, Any]) -> Dict[str, Any]:
        """Normalize query params for Gamma API.

        Gamma endpoints generally accept booleans as true/false strings.
        """
        out: Dict[str, Any] = {}
    ...
    def list_markets(self, **params) -> List[Dict[str, Any]]:
        """List markets using the Gamma /markets endpoint."""
        url = f"{self.host}/markets"
        try:
            response = self.session.get(url, params=self._normalize_params(params), timeout=self.timeout)
    ...
    def list_events(self, use_pagination: bool = False, **params) -> List[Dict[str, Any]]:
        """List events using Gamma /events (or /events/pagination) endpoint.

        Supports tag_slug, active, archived, closed, order, ascending, limit, offset.
        """
        path = "/events/pagination" if use_pagination else "/events"
    ...
    def get_market_by_slug(self, slug: str) -> Optional[Dict[str, Any]]:
        ...
        Polymarket has both "event" slugs (website URLs) and Gamma "market" slugs.
        For many binary markets they match, but not always.

        Strategy:
          1) Try the direct Gamma slug endpoint: /markets/slug/{slug}
          2) Fallback to listing markets filtered by slug (if supported)
          3) Fallback to searching/listing markets and picking the best slug match
        """
        ...
        # 2) Filtered list fallback
        for m in self.list_markets(slug=slug, limit=5, offset=0):
            if str(m.get("slug") or "").strip() == slug:
                return m

        # 3) Search/list fallback with a few common query parameter names
        variants = [
            {"search": slug, "limit": 20, "offset": 0, "order": "createdAt", "ascending": False},
            {"query": slug, "limit": 20, "offset": 0, "order": "createdAt", "ascending": False},
            {"term": slug, "limit": 20, "offset": 0, "order": "createdAt", "ascending": False},
        ]

        hits: List[Dict[str, Any]] = []
        for params in variants:
            res = self.list_markets(**params)
            if res:
                hits.extend(res)
        ...
        def score(mkt: Dict[str, Any]) -> tuple:
            s = str(mkt.get("slug") or "")
            if s == slug:
                return (0, 0)
            if slug in s:
                return (1, abs(len(s) - len(slug)))
            return (2, abs(len(s) - len(slug)))
