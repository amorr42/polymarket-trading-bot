...
from lib.market_selector import DbPrefixSelector
...
def main():
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Requires: python apps/ingest_markets_pg.py --source markets --keyword <prefix>"
        ),
    )
    parser.add_argument(
        "--debug",
    ...
    # Resolve market selector
    db_prefix = args.db_prefix.strip()
    selector = DbPrefixSelector(db_prefix) if db_prefix else None
    display_name = db_prefix if db_prefix else f"{args.coin.upper()} 15-Minute"
    ...
    print(f"{Colors.BOLD}  Flash Crash Strategy - {display_name} Markets{Colors.RESET}")
