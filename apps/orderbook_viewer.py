...
from lib import MarketManager, PriceTracker, Colors, FlashCrashEvent, DbPrefixSelector
...
class OrderbookTUI:
    ...
    def __init__(self, coin: str = "ETH", db_prefix: str = ""):
        """Initialize TUI."""
        self.coin = coin.upper() if not db_prefix else db_prefix
        if db_prefix:
            selector = DbPrefixSelector(db_prefix)
    ...
    def render(self) -> None:
        ...
        up_label = self.market.labels.get("up", "UP")
        down_label = self.market.labels.get("down", "DOWN")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Orderbook TUI for Polymarket markets"
    )
    parser.add_argument(
        "--coin",
        type=str,
        ...
        help="Coin to monitor via Gamma API (default: ETH)"
    )
    parser.add_argument(
        "--db-prefix",
        type=str,
        ...
        dest="db_prefix",
        help=(
            "Slug prefix to look up in the local DB (e.g. btc-updown-5m). "
            "When set, --coin is ignored. "
            "Requires markets to be ingested first: "
            "python apps/ingest_markets_pg.py --source markets --keyword <prefix>"
        ),
    )

    args = parser.parse_args()

    tui = OrderbookTUI(coin=args.coin, db_prefix=args.db_prefix)
