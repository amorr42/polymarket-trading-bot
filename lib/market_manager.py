...
class MarketManager:
    ...
    def __init__(
        ...
        selector: MarketSelector | None = None,
    ):
        """
        ...
        """
        # Backwards compatible: if selector isn't provided, use 15m coin selector.
        self.selector: MarketSelector = selector or FifteenMinuteCoinSelector(coin=coin)

        self.market_check_interval = market_check_interval
        # Only enable auto-switching if selector supports it.
        self.auto_switch_market = auto_switch_market and self.selector.supports_auto_switch

        # Clients (kept for compatibility / future usage)
        ...
        # Human-friendly startup errors (e.g. slug not found / non-binary market)
        self.last_error: str = ""
    ...
    def labels(self) -> Dict[str, str]:
        """Display labels for the two sides (e.g. UP/DOWN or YES/NO)."""
        if self.current_market and self.current_market.labels:
            return self.current_market.labels
    ...
    def up_token(self) -> str:
        """Get UP token ID."""
        if self.current_market:
            return self.current_market.up_token
        return ""
    ...
    def down_token(self) -> str:
        """Get DOWN token ID."""
        if self.current_market:
            return self.current_market.down_token
        return ""
    ...
    def discover_market(self, update_state: bool = True) -> Optional[MarketInfo]:
        """Discover/select the current market using the configured selector."""
        selected: Optional[SelectedMarket] = self.selector.select()
        if not selected:
            # Pull a useful error message if the selector provides one.
            self.last_error = getattr(self.selector, "last_error", "") or "No market found for the given selection."
            return None
        if not selected.accepting_orders:
            self.last_error = "Market found but it is not accepting orders (inactive/paused)."
            return None
